<script>
    ...
    $: data = [...Array(10)].map((_, idx) => {
      ...
      const avgPrice = (($stockData.curAvgPrice*$stockData.curQuantity) + newAmt)/($stockData.curQuantity + (newAmt/$stockData.newPrice))
    ...
    $:newAvgCost = (($stockData.curAvgPrice*$stockData.curQuantity) + $stockData.newTotalCost)/($stockData.curQuantity + ($stockData.newTotalCost/$stockData.newPrice))
</script>
...
<div class='row'>
    <div class='col lg-3 sm-6'>
        ...
        <h5>Stock Name</h5>
        <input placeholder="Stock Name" class="margin-bottom" bind:value={$stockData.stockName}/>
        
        <h5>Average Price</h5>
        <input type='number' placeholder="Average Price" bind:value={$stockData.curAvgPrice} class="margin-bottom"/>

        <h5>Quantity</h5>  
        <input type='number' placeholder="Quantity" bind:value={$stockData.curQuantity} />
        </Card>
    </div>  
    <div class='col lg-3 sm-6'>
