<script>
  ...
  import { Card, Checkbox, Input, Radio, Table } from "spaper";
  ...
  <div class="col lg-6 sm-12">
    <Card>
      <fieldset class="form-group">
        <Checkbox label="Show Graph" bind:checked={show} />
      </fieldset>
      <div style="width:500px;">
        <Linechart {show} data={data.map(d=>d["Price difference"])} />
      </div>
    </Card>
  </div>
  <div class="col lg-6 sm-12">
    <Card>
      ...
      <div style="width:500px;">
        <Linechart {show} data={data.map(d=>d["Average Price"])} />
      </div>
    </Card>
  </div>
</div>
